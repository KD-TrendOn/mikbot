...
async def chat(user_input: UserInput, db: AsyncSession = Depends(get_db)):
    try:
        ...
        await create_chat_message(
            db, user_input.user_id, "user", {"message": user_input.user_input}
        )
        ...
        if isinstance(pre_answer, ToolMessage):
            ...
            return {"tool_call": resultt, "type": tipe, "message": description}
        else:
            ...
            await create_chat_message(
                ...
                user_input.user_id,
                "bot",
                {"service": result.get("service"), "message": answer},
            )
            return {"tool_call": {}, "type": "text", "message": answer}
