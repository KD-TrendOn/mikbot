...
async def load_tools(db: AsyncSession, service_name: str) -> List[ToolWrapper]:
    ...
    return [
        ...
        for tool in tools
        if tool.function and tool.input_schema
    ]


async def save_tool(
    db: AsyncSession, tool: ToolWrapper, service_id: int
) -> models.Tool:
    db_tool = models.Tool(
        name=tool.name,
        description=tool.description,
    ...
    print("Добавили", db_tool)
    return db_tool


async def save_service(
    db: AsyncSession, service_name: str, prompt: str, documentation: str
) -> models.Service:
    db_service = models.Service(
        name=service_name, prompt=prompt, documentation=documentation
    )
    db.add(db_service)
    await db.commit()
...
async def update_service(
    db: AsyncSession, service_id: int, prompt: str = None, documentation: str = None
) -> Optional[models.Service]:
...
async def load_vector_documents(service_name: str, query: str, k: int = 3):
    vectorstore = get_vectorstore()
    docs = await vectorstore.asimilarity_search_with_score(
        query, k=k, filter={"service": service_name}
    )
    return docs


async def create_chat_message(
    db: AsyncSession, user_id: str, sender_role: str, content: Dict[str, Any]
) -> models.ChatMessage:
    db_message = models.ChatMessage(
        user_id=user_id, sender_role=sender_role, content=content
    )
    db.add(db_message)
    await db.commit()
